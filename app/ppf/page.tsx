...
// app/kaca-film/page.tsx, cuma kontennya di-generalize ke PPF saja —
// SENGAJA berdiri sendiri (bukan share komponen) mengikuti konvensi
// /partner & /giias di repo ini.
const TITLE = 'Ginnva Paint Protection Film — Perlindungan Cat Total';
const DESC = 'Distributor resmi dan perwakilan eksklusif Ginnva di Indonesia — Paint Protection Film premium TPU 3rd Generation, self-healing, garansi hingga 8 tahun, dipasang presisi di GINNVA House, PIK 2.';
...
export default function PpfPage() {
  return (
    <main data-page="ppf" style={{ backgroundColor: '#fff' }}>
      {/* ================= HERO + NAVBAR BLUR ================= */}
      <section
        style={{
          position: 'relative',
      ...
      >
        ...
        >
          <div className="wrap" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '64px' }}>
            ...
            <a className="pill pill--accent" style={{ height: '38px', padding: '0 20px', fontSize: '13px' }} href={waLink('Halo Ginnva, saya tertarik konsultasi PPF')} target="_blank" rel="noopener">
              Chat WhatsApp
            </a>
          </div>
        </header>
        ...
        <div style={{ maxWidth: '720px', margin: '0 auto', position: 'relative', zIndex: 2 }}>
          ...
          <p style={{ fontSize: 'clamp(15px, 1.6vw, 18px)', color: 'rgba(255,255,255,.85)', marginTop: '18px', maxWidth: '56ch', marginInline: 'auto' }}>
            Distributor resmi dan perwakilan eksklusif Ginnva di Indonesia — Paint Protection Film premium
            TPU 3rd Generation, self-healing, garansi hingga 8 tahun, dipasang presisi di GINNVA House, PIK 2.
          </p>
          <a
            href={waLink('Halo Ginnva, saya tertarik konsultasi PPF')}
            target="_blank"
            rel="noopener"
      ...
      <Section alt>
        ...
        <div className={styles.grid2} style={{ alignItems: 'stretch' }}>
          <div className={styles.cmpCard}>
            <div className={styles.cmpLabel}>Tanpa PPF</div>
            <ul>
              {['Goresan kunci & kuku menumpuk', 'Cat kusam & menguning', 'Permukaan sulit dibersihkan', 'Nilai jual kembali turun'].map((t) => (
                <li key={t}><XCircle color="#c0392b" /> {t}</li>
              ))}
            </ul>
          </div>
          <div className={`${styles.cmpCard} ${styles.cmpCardWith}`}>
            <div className={styles.cmpLabel}>Dengan Ginnva PPF</div>
            <ul>
              {['Terlindungi dari goresan harian', 'Cat tetap glossy & jernih', 'Mudah dibersihkan dari noda', 'Nilai jual kembali terjaga'].map((t) => (
