...
// Sama polanya dengan BecomePartnerBar.tsx di /giias, beda cuma endpoint
// (/api/partner-signup — link referral yang dihasilkan mengarah ke
// /partner, bukan /giias) dan copy yang tidak menyebut event tertentu.
export default function PartnerSignupBar({ hasReferralCode }: { hasReferralCode: boolean }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/partner-signup`, {
        method: 'POST',
    ...
    <>
      ...
      >
        <span style={{ fontSize: '13.5px', color: 'rgba(255,255,255,.75)' }}>
          Kamu Sales mobil? Dapatkan link referral Anda sendiri.
        </span>
        <button
          type="button"
